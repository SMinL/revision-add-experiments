...
if __name__ == '__main__':
    all_examples = json.load(open('5/train_sql_spider_style_choose_schema_rat.json', 'r'))
    all_examples = json.load(open('5/dev_sql_spider_style_choose_schema_rat.json', 'r'))
    all_examples = json.load(open('5/test_sql_spider_style_choose_schema_rat.json', 'r'))
    ...
    for i,example in enumerate(all_examples):
        ...
        print('*'*100)
        vql = example['VQL']
        predefine_schema = example['choose_schema_rat']
        print('vql:',vql)
        ...
        for content in separate_predefine_schemas:
            table = content.split('columns')[0].strip().replace(',','').replace('Table','').strip()
            columns = content.split('columns')[1].replace('= [','').replace(']','')
        ...
        print('current_selected_tables',current_selected_tables)
        print('current_selected_columns',current_selected_columns)
        # exit(0)
        all_selected_tables.append(current_selected_tables)
        all_selected_schemas.append(current_selected_columns)
        schemas = [sc for sc in predefine_schema.split('@sep@')]
