...
import copy
prompt_thought_v2 = '''
# Question: Can you draw the trend of the average of capacity over the openning year ? , and sort by the x axis in desc .
...
The question ask 'who have the letters D or S in their first name', so we need 'where' clause.
The question ask 'bin hire_date by weekday', so we need 'bin' clause.
The question ask 'order by the y-axis from high to low', so we need 'order' clause. Therefore, to answer this question, we need data transformations : where, bin, order
