...
if __name__ == '__main__':
    ...
    paths = ['../source-data-for-prompt4vis/source-data-and-results-in-experiments-fold-1/test-results-prompt4vis/questions.json']
    for path in paths:
        ...
        for i, (tgt, pred) in enumerate(zip(all_target_responses, predictions)):
            tgt = tgt.lower()
            pred = pred.lower()
            tgt_list = [t.strip() for t in tgt.split()]
            if ('order' in tgt_list) or ('group' in tgt_list) or ('bin' in tgt_list) or ('order by' in tgt_list) or ('where' in tgt_list):
                data_transformation_tgts.append(tgt)
                data_transformation_preds.append(pred)
            else:
                wo_data_trans_tgts.append(tgt)
                wo_data_trans_preds.append(pred)

        print('data_transformation case percent:',(len(data_transformation_tgts)/float(len(all_target_responses)))*100)

        acc_tree1, acc_vis1, acc_axis1, acc_data1, trees, viss, axiss, datas = tree_accuracy(data_transformation_preds,
                                                                                             data_transformation_tgts)
        print('accuracy with data transformation:')
        print(acc_tree1, acc_vis1, acc_axis1, acc_data1)
        acc_tree1, acc_vis1, acc_axis1, acc_data1, trees, viss, axiss, datas = tree_accuracy(wo_data_trans_preds,wo_data_trans_tgts)
        print('accuracy with data transformation:')
        print(acc_tree1, acc_vis1, acc_axis1, acc_data1)
