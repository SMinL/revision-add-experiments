...
if __name__ == '__main__':
    ...
    for path in paths:
        ...
        bin_fail_cases = []
        order_fail_cases = []
        for i, (tgt, pred, data) in enumerate(zip(all_target_responses, predictions, datas)):
            if 'wrong' in data:

                data_fail_cases.append(data)
                if data[2]!=data[4]:
                    where_fail_cases.append(data)
                    # print('*' * 100)
                    # print('data', data)
                    # print('tgt', tgt)
                    # print('pred', pred)
                ...
                if data[6]!=data[8]:
                    group_fail_cases.append(data)

                if data[10]!=data[12]:
                    bin_fail_cases.append(data)
                if data[14]!=data[16]:
                    order_fail_cases.append(data)
                    print('*' * 100)
                    print('data', data)
                    print('tgt', tgt)
                    print('pred', pred)
                    print(prompt_examples[i])
        print('where:',(len(where_fail_cases)/float(len(data_fail_cases)))*100)
        print('group:', (len(group_fail_cases) / float(len(data_fail_cases)))*100)
        print('bin:', (len(bin_fail_cases) / float(len(data_fail_cases)))*100)
        print('order:', (len(order_fail_cases) / float(len(data_fail_cases)))*100)
